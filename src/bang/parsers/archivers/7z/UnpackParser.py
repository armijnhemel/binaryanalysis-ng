...
class SevenzipUnpackParser(UnpackParser):
    ...
    def unpack(self, meta_directory):
        ...
        if not self.encrypted:
            ...
            if self.havetmpfile:
                p = subprocess.Popen(['7z', '-o%s' % unpackdir_full, '-y', 'x', self.temporary_file[1]], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            else:
                p = subprocess.Popen(['7z', '-o%s' % unpackdir_full, '-y', 'x', self.infile.name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
