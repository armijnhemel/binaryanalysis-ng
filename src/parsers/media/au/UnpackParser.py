...
class AuUnpackParser(UnpackParser):
    ...
    def parse(self):
        ...
        check_condition(self.data.data_size != 0xffffffff,
                        "files with unknown data size not supported")
        check_condition(self.file_size >= self.data.header_size + self.data.data_size,
                        "not enough data")

    def calculate_unpacked_size(self):
        self.unpacked_size = self.data.header_size + self.data.data_size
